...
async function getAccessToken(projectIdFromEnv?: string) {
  ...
  if (!projectId) {
    throw new Error(
      "Missing project_id in service account or env FIREBASE_PROJECT_ID."
    );
  }
...
export async function sendFcmMessage(options: SendFcmOptions) {
  ...
  if (!res.ok) {
    ...
    // Ignore unregistered/expired tokens to keep flow running
    try {
      ...
      if (code === "UNREGISTERED" || code === "NOT_FOUND") {
        console.warn("[FCM] Skipping unregistered token/topic");
